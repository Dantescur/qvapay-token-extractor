...
api.post('/login', async (c) => {
  ...
  if (!email || !password) return c.json({ error: 'Email y contraseña requeridos' }, 400 as ContentfulStatusCode)
  ...
  switch (result.type) {
    case '2fa-required':
      return c.json({ requires2FA: true, has_otp: result.has_otp, sessionId: result.sessionId, notified: result.notified })
    case 'success':
