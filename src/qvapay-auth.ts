...
export function createQvapayAuth(deps: QvapayAuthDeps) {
  async function login(email: string, password: string, otpCode?: string): Promise<LoginResult> {
    const res = await fetch(`${deps.qvapayApiUrl}/auth/login`, {
      ...
      body: JSON.stringify({ email, password, remember: true, ...(otpCode && { two_factor_code: otpCode }) }),
    })
    ...
    return { type: 'error', message: data.message || 'Credenciales inválidas', status: (res.status === 401 ? 401 : 400) as ContentfulStatusCode }
  }
  ...
  async function requestPin(sessionId: string): Promise<RequestPinResult> {
    ...
    return { type: 'error', message: data.message || 'No se pudo enviar el PIN', status: (res.status === 401 ? 401 : res.status === 429 ? 429 : 400) as ContentfulStatusCode }
  }
